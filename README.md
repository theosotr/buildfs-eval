...
Note that we have two separate images, because there are several difficulties
in having a `gradle` and `sbuild` installation on the same image.

**NOTE:** The second image (`buildfs-sbuild`) is used **only** for
...
After building/downloading all Docker images successfully,
...
and detecting faults using `mkcheck`.
...
the `out` directory contains a sub-directory for each project.
...
This file was *not* computed. We copied it
...
Also, this version is taken from the `project_version` column of
...
* Two projects (namely `helios` and `LuaJIT`) are expected to produce entries
  ...
  the issue still remains even after the applying our fix. The reason is that
...
To dump the metrics mention in RQ4 of the paper, run
...
Now you are able to compute the slowdown that `BuildFS` imposes on the build
...
The script also dumps some metrics regarding the speedup of `BuildFS`
over `mkcheck`.
...
To re-run Gradle builds, execute (estimated running times: 4-6 days)
...
the third argument indicates whether the script runs the builds wihout
`BuildFS` as well (i.e., the value `1` runs the build both with and without
...
(estimated running times: 3-5 days)
...
message produced by `sbuild`. The analysis and the build of the project
was still done successfully.
